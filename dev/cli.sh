...
function ensure_config {
    CONFIG_DIR="$HOST_TARGET_DIR/.multiclaw"
    CONFIG_FILE="$CONFIG_DIR/config.toml"
    WORKSPACE_DIR="$CONFIG_DIR/workspace"

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚙️  Config file missing in target/.multiclaw. Creating default dev config from template...${NC}"
...
function print_help {
    echo -e "${YELLOW}MultiClaw Development Environment Manager${NC}"
    ...
    echo -e "  ${GREEN}agent${NC}   Enter Agent (MultiClaw CLI)"
...
case "$1" in
    up)
        ...
        echo -e "   - Config: target/.multiclaw/config.toml (Edit locally to apply changes)"
        ;;
    ...
    shell)
        ...
        docker exec -it multiclaw-sandbox /bin/bash
        ;;

    agent)
        echo -e "${GREEN}🤖 Entering Agent Container (MultiClaw)... (Type 'exit' to leave)${NC}"
        docker exec -it multiclaw-dev /bin/bash
        ;;
    ...
    clean)
        echo -e "${RED}⚠️  WARNING: This will delete 'target/.multiclaw' data and Docker volumes.${NC}"
        read -p "Are you sure? (y/N) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker compose -f "$COMPOSE_FILE" down -v
            rm -rf "$HOST_TARGET_DIR/.multiclaw"
