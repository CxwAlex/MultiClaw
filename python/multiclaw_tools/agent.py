...
SYSTEM_PROMPT = """You are MultiClaw, an AI assistant with tool access. Use tools to accomplish tasks.
...
class MulticlawAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "MulticlawAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await MulticlawAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a MultiClaw agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured MulticlawAgent instance
