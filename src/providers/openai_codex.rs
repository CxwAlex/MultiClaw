...
fn resolve_reasoning_effort(model_id: &str, override_level: Option<&str>) -> String {
    ...
    let env_level = std::env::var("MULTICLAW_CODEX_REASONING_EFFORT")
        .ok()
        .and_then(|value| {
            normalize_reasoning_level(Some(&value), "MULTICLAW_CODEX_REASONING_EFFORT")
        });
...
mod tests {
    ...
    fn resolve_reasoning_effort_prefers_config_override() {
        let _env_lock = env_lock();
        let _reasoning_guard = EnvGuard::set("MULTICLAW_CODEX_REASONING_EFFORT", Some("low"));
    ...
    fn resolve_reasoning_effort_falls_back_to_env_when_override_invalid() {
        let _env_lock = env_lock();
        let _reasoning_guard = EnvGuard::set("MULTICLAW_CODEX_REASONING_EFFORT", Some("medium"));
