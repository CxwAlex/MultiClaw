//! Flash MultiClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash MultiClaw Nucleo firmware. Builds from firmware/multiclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (multiclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("multiclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from multiclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building MultiClaw Nucleo firmware...");
    ...
    println!("MultiClaw Nucleo firmware flashed successfully.");
