//! Flash MultiClaw Arduino firmware via arduino-cli.
//!
...
/// MultiClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/multiclaw-arduino/multiclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "multiclaw-arduino";
...
/// Flash MultiClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("multiclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling MultiClaw Arduino firmware...");
    ...
    println!("MultiClaw firmware flashed successfully.");
