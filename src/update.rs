//! Self-update functionality for MultiClaw.
//!
...
const GITHUB_REPO: &str = "multiclaw-labs/multiclaw";
const GITHUB_API_RELEASES: &str =
    "https://api.github.com/repos/multiclaw-labs/multiclaw/releases/latest";
...
fn get_archive_name(target: &str) -> String {
    if target.contains("windows") {
        format!("multiclaw-{}.zip", target)
    } else {
        format!("multiclaw-{}.tar.gz", target)
    }
}
...
async fn fetch_latest_release() -> Result<Release> {
    let client = reqwest::Client::builder()
        .user_agent(format!("multiclaw/{}", current_version()))
        .build()
...
pub async fn self_update(force: bool, check_only: bool) -> Result<()> {
    ...
    if check_only {
        ...
        println!("Run `multiclaw update` to install the update.");
        return Ok(());
    }
    ...
    println!();
    println!("Restart MultiClaw to use the new version.");
