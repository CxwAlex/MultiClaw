...
///
/// When enabled, registers IPC tools that let independent MultiClaw processes
...
pub struct WasmConfig {
    ...
    pub fuel_limit: u64,
    /// URL of the ZeroMarket (or compatible) registry used by `multiclaw skill install`.
...
pub struct HttpRequestConfig {
    ...
    /// User-Agent string sent with HTTP requests (env: MULTICLAW_HTTP_REQUEST_USER_AGENT)
...
pub struct WebFetchConfig {
    ...
    pub timeout_secs: u64,
    /// User-Agent string sent with fetch requests (env: MULTICLAW_WEB_FETCH_USER_AGENT)
    #[serde(default = "default_user_agent")]
    pub user_agent: String,
}
...
pub struct WebSearchConfig {
    ...
    pub timeout_secs: u64,
    /// User-Agent string sent with search requests (env: MULTICLAW_WEB_SEARCH_USER_AGENT)
    #[serde(default = "default_user_agent")]
    pub user_agent: String,
}
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "multiclaw" | "internal" | "core" => Some(ProxyScope::Multiclaw),
...
pub struct PluginsConfig {
    ...
    /// Standard locations: `<binary_dir>/extensions/`, `~/.multiclaw/extensions/`,
    /// `<workspace>/.multiclaw/extensions/`.
    #[serde(default)]
...
pub struct SyscallAnomalyConfig {
    ...
    /// Path to syscall anomaly log file (relative to ~/.multiclaw unless absolute).
...
impl Config {
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        // Skills script-file audit override: MULTICLAW_SKILLS_ALLOW_SCRIPTS
        if let Ok(flag) = std::env::var("MULTICLAW_SKILLS_ALLOW_SCRIPTS") {
            if !flag.trim().is_empty() {
                match flag.trim().to_ascii_lowercase().as_str() {
                    "1" | "true" | "yes" | "on" => self.skills.allow_scripts = true,
                    "0" | "false" | "no" | "off" => self.skills.allow_scripts = false,
                    _ => tracing::warn!(
                        "Ignoring invalid MULTICLAW_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                    ),
                }
            }
        }
        ...
        // Deprecated reasoning level alias: MULTICLAW_REASONING_LEVEL or REASONING_LEVEL
        let alias_level = std::env::var("MULTICLAW_REASONING_LEVEL")
            .ok()
            .map(|value| ("MULTICLAW_REASONING_LEVEL", value))
            .or_else(|| {
        ...
        // Vision support override: MULTICLAW_MODEL_SUPPORT_VISION or MODEL_SUPPORT_VISION
        if let Ok(flag) = std::env::var("MULTICLAW_MODEL_SUPPORT_VISION")
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        std::env::set_var("MULTICLAW_SKILLS_ALLOW_SCRIPTS", "yes");
        ...
        std::env::remove_var("MULTICLAW_SKILLS_ALLOW_SCRIPTS");
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        std::env::set_var("MULTICLAW_SKILLS_ALLOW_SCRIPTS", "maybe");
...
"#,
    ...
    async fn env_override_reasoning_level_alias() {
        ...
        std::env::set_var("MULTICLAW_REASONING_LEVEL", "xhigh");
        config.apply_env_overrides();
        ...
        std::env::remove_var("MULTICLAW_REASONING_LEVEL");
    }
    ...
    async fn env_override_reasoning_level_alias_invalid_ignored() {
        ...
        std::env::set_var("MULTICLAW_REASONING_LEVEL", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_model_support_vision() {
        ...
        std::env::set_var("MULTICLAW_MODEL_SUPPORT_VISION", "true");
        config.apply_env_overrides();
        assert_eq!(config.model_support_vision, Some(true));

        std::env::set_var("MULTICLAW_MODEL_SUPPORT_VISION", "false");
        config.apply_env_overrides();
        assert_eq!(config.model_support_vision, Some(false));

        std::env::set_var("MULTICLAW_MODEL_SUPPORT_VISION", "maybe");
        config.model_support_vision = Some(true);
        config.apply_env_overrides();
        assert_eq!(config.model_support_vision, Some(true));

        std::env::remove_var("MULTICLAW_MODEL_SUPPORT_VISION");
    }
