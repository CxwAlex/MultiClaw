//! Plugin manifest — the `multiclaw.plugin.toml` descriptor.
//!
//! Mirrors OpenClaw's `openclaw.plugin.json` but uses TOML to match
//! MultiClaw's existing config format.
...
pub const PLUGIN_MANIFEST_FILENAME: &str = "multiclaw.plugin.toml";
...
/// Load and parse `multiclaw.plugin.toml` from `root_dir`.
