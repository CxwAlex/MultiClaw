...
//! extension directories for subdirectories containing `multiclaw.plugin.toml`.
...
pub fn discover_plugins(
    ...
    if let Some(home) = dirs_home() {
        let global = home.join(".multiclaw").join("extensions");
    ...
    if let Some(ws) = workspace_dir {
        let ws_ext = ws.join(".multiclaw").join("extensions");
...
"#
    ...
    fn discover_from_workspace() {
        ...
        let ext_dir = ws.join(".multiclaw").join("extensions");
