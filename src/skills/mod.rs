...
pub fn scaffold_skill(
    ...
    let tmpl = templates::find(template_name).ok_or_else(|| {
        ...
        anyhow::anyhow!(
            "Unknown template '{template_name}'. Run 'multiclaw skill templates' to list available templates.\nAvailable: {}",
            names.join(", ")
        )
    })?;
    ...
    std::fs::write(
        ...
        format!(
             ...
             **Example:**\n\
             ...
             ## Test\n\n\
             ```bash\n\
             multiclaw skill test . --args '{test_args}'\n\
             ```\n"
        ),
    )?;
    Ok(())
...
fn write_readme(dir: &std::path::Path, name: &str, language: &str, test_args: &str) -> Result<()> {
    let (build_cmd, test_note) = match language {
        ...
        "python" => (
            "componentize-py -d wit/ -w multiclaw-skill componentize main -o tool.wasm",
...
// ─── Local test (multiclaw skill test) ────────────────────────────────────────
...
///
/// Usage: `multiclaw skill install /path/to/skill.zip`
...
fn extract_zip_bytes_to_skills(
    ...
    if skill_dir.exists() {
        anyhow::bail!(
            "skill '{}' already exists at {}; run 'multiclaw skill remove {}' first",
            skill_name,
            skill_dir.display(),
            skill_name
        );
    ...
    // Write a minimal SKILL.toml so the skill appears in `multiclaw skill list`
    ...
    match command {
        crate::SkillCommands::New { name, template } => {
            ...
            match tmpl.language {
                ...
                "python" => {
                    ...
                    println!("    componentize-py -d wit/ -w multiclaw-skill componentize main -o tool.wasm");
                }
                _ => {}
            }
            println!("    multiclaw skill test . --args '{}'", tmpl.test_args);
            println!();
            println!(
                "  {} 'multiclaw skill test' requires the {} CLI:",
        ...
        crate::SkillCommands::Install { source } => {
            ...
            if is_clawhub_source(&source) {
                ...
                println!("  Run 'multiclaw skill list' to verify the new tools are available.");
        ...
        crate::SkillCommands::Templates => {
            ...
            println!();
            println!("  Usage:");
            println!("    multiclaw skill new <name> --template <template-name>");
            println!();
            println!("  Example:");
            println!(
                "    multiclaw skill new my_weather --template {}",
...
"#,
    ...
    fn registry_install_dir_name_is_package_name_only() {
        ...
        for (source, expected_dir) in [
            ("multiclaw-org/weather-lookup", "weather-lookup"),
            ("multiclaw-org/calculator", "calculator"),
            ("multiclaw-user/my_tool", "my_tool"),
        ] {
    ...
    fn is_registry_source_accepts_valid_namespace_name() {
        assert!(is_registry_source("multiclaw/weather-lookup"));
    ...
    fn scaffold_skill_rejects_unknown_template() {
        let dir = tempfile::tempdir().unwrap();
        let result = scaffold_skill("multiclaw_test_tool", "cobol", dir.path());
        assert!(result.is_err());
        let msg = result.unwrap_err().to_string();
    ...
    fn scaffold_skill_rejects_existing_directory() {
        let dir = tempfile::tempdir().unwrap();
        fs::create_dir_all(dir.path().join("multiclaw_test_tool")).unwrap();
        let result = scaffold_skill("multiclaw_test_tool", "typescript", dir.path());
        assert!(result.is_err());
        let msg = result.unwrap_err().to_string();
    ...
    fn scaffold_skill_typescript_creates_required_files() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("multiclaw_test_ts", "typescript", dir.path()).unwrap();
        let skill_dir = dir.path().join("multiclaw_test_ts");
    ...
    fn scaffold_skill_rust_creates_required_files() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("multiclaw_test_rs", "rust", dir.path()).unwrap();
        let skill_dir = dir.path().join("multiclaw_test_rs");
    ...
    fn scaffold_skill_substitutes_name_placeholder() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("multiclaw_subst_test", "rust", dir.path()).unwrap();
        let cargo_toml =
            fs::read_to_string(dir.path().join("multiclaw_subst_test").join("Cargo.toml")).unwrap();
        assert!(
            cargo_toml.contains("multiclaw_subst_test"),
    ...
    fn scaffold_skill_go_creates_required_files() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("multiclaw_test_go", "go", dir.path()).unwrap();
        let skill_dir = dir.path().join("multiclaw_test_go");
        assert!(
            skill_dir.join("manifest.json").exists(),
            "manifest.json missing"
        );
    ...
    fn scaffold_skill_gitignore_always_created() {
        for template in ["rust", "typescript", "go", "python"] {
            let dir = tempfile::tempdir().unwrap();
            let name = format!("multiclaw_test_{template}");
    ...
    fn scaffold_skill_skill_md_contains_name() {
        let dir = tempfile::tempdir().unwrap();
        scaffold_skill("multiclaw_md_check", "typescript", dir.path()).unwrap();
        let skill_md =
            fs::read_to_string(dir.path().join("multiclaw_md_check").join("SKILL.md")).unwrap();
        assert!(
            skill_md.contains("multiclaw_md_check"),
    ...
    fn is_clawhub_source_rejects_other_domains() {
        ...
        assert!(!is_clawhub_source("multiclaw/skill"));
    }
    ...
    fn is_zip_url_source_rejects_other_formats() {
        assert!(!is_zip_url_source("multiclaw/skill"));
