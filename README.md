...
<p align="center">
  ...
  <a href="NOTICE"><img src="https://img.shields.io/github/contributors/multiclaw-labs/multiclaw?color=green" alt="Contributors" /></a>
...
```bash
curl -LsSf https://raw.githubusercontent.com/multiclaw-labs/multiclaw/main/scripts/install.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/multiclaw-labs/multiclaw/main/scripts/install-release.sh | bash
```
...
- Installs `multiclaw` into a local bin directory (or `/usr/local/bin` if needed)
- Starts `multiclaw onboard` (skip with `--no-onboard`)
...
curl -fsSL https://raw.githubusercontent.com/multiclaw-labs/multiclaw/main/scripts/install-release.sh | bash -s -- --no-onboard
```
...
<https://github.com/multiclaw-labs/multiclaw/releases/latest>

Example (ARM64 Linux):

```bash
curl -fsSLO https://github.com/multiclaw-labs/multiclaw/releases/latest/download/multiclaw-aarch64-unknown-linux-gnu.tar.gz
...
MultiClaw supports WASM-compiled skills installable from the [ZeroMarket](https://zeromarket.vercel.app) registry and zip-based registries like [ClawhHub](https://clawhub.ai):

```bash
# Install from ZeroMarket registry
multiclaw skill install namespace/name

# Install from ClawhHub (auto-detected by domain)
multiclaw skill install https://clawhub.ai/steipete/summarize

# Install using ClawhHub short prefix
multiclaw skill install clawhub:summarize

# Install from a zip file already downloaded locally
multiclaw skill install ~/Downloads/summarize-1.0.0.zip

# Install from any direct zip URL
multiclaw skill install zip:https://example.com/my-skill.zip
```

If ClawhHub returns 429 (rate limit) or requires authentication, add to `~/.multiclaw/config.toml`:
...
Skills are installed to `~/.multiclaw/workspace/skills/<name>/` and loaded automatically as tools at agent runtime. No system `unzip` binary required — zip extraction is handled in-process.
...
Publish your own skill to ZeroMarket: compile to WASM, upload `tool.wasm`, `manifest.json`, and `SKILL.md` via the ZeroMarket upload page. Use `multiclaw skill new <name>` to scaffold a new skill project.
...
> https://github.com/multiclaw-labs/multiclaw
...
If you encounter impersonation or trademark misuse, please [open an issue](https://github.com/multiclaw-labs/multiclaw/issues).
...
New to MultiClaw? Look for issues labeled [`good first issue`](https://github.com/multiclaw-labs/multiclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — see our [Contributing Guide](CONTRIBUTING.md#first-time-contributors) for how to get started.
...
<p align="center">
  <a href="https://www.star-history.com/#multiclaw-labs/multiclaw&type=date&legend=top-left">
    <picture>
     <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=multiclaw-labs/multiclaw&type=date&theme=dark&legend=top-left" />
     <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=multiclaw-labs/multiclaw&type=date&legend=top-left" />
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=multiclaw-labs/multiclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
