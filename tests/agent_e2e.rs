...
//!
//! Ref: https://github.com/multiclaw-labs/multiclaw/issues/618 (item 6)
...
///
/// Requires valid credentials in `~/.multiclaw/`.
...
async fn e2e_live_research_phase() {
    use std::sync::Arc;
    use multiclaw::agent::research::{run_research_phase, should_trigger};
    use multiclaw::config::{ResearchPhaseConfig, ResearchTrigger};
    use multiclaw::observability::NoopObserver;
    use multiclaw::providers::openai_codex::OpenAiCodexProvider;
    use multiclaw::providers::traits::Provider;
    use multiclaw::tools::{Tool, ToolResult};
    ...
    let observer: Arc<dyn multiclaw::observability::Observer> = Arc::new(NoopObserver);
...
async fn e2e_agent_research_prompt_guided() {
    use multiclaw::config::{ResearchPhaseConfig, ResearchTrigger};
    use multiclaw::providers::traits::ProviderCapabilities;
