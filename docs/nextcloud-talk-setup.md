...
This guide covers native Nextcloud Talk integration for MultiClaw.
...
Add this section in `~/.multiclaw/config.toml`:
...
multiclaw daemon
# or
multiclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, MultiClaw verifies:
...
- MultiClaw ignores bot-originated webhook events (`actorType = bots`).
- MultiClaw ignores non-message/system events.
...
3. Confirm MultiClaw receives and replies in the same room.
