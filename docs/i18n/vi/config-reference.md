...
- Biến môi trường: `MULTICLAW_MODEL_SUPPORT_VISION` hoặc `MODEL_SUPPORT_VISION` (giá trị: `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off`).
...
- Khi đặt, ghi đè `MULTICLAW_CODEX_REASONING_EFFORT` cho OpenAI Codex.
- Để trống sẽ dùng `MULTICLAW_CODEX_REASONING_EFFORT` nếu có, nếu không mặc định `xhigh`.
