...
Vì MultiClaw không thể tự động phát hiện, bạn có thể ghi đè trong `config.toml`:
...
Biến môi trường: `MULTICLAW_MODEL_SUPPORT_VISION=true`
...
- Khi đặt, ghi đè `MULTICLAW_CODEX_REASONING_EFFORT`.
- Không đặt sẽ dùng `MULTICLAW_CODEX_REASONING_EFFORT` nếu có, nếu không mặc định `xhigh`.
