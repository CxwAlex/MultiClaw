...
Nếu `[channels_config.matrix]`, `[channels_config.lark]`, hoặc `[channels_config.feishu]` có mặt nhưng binary được build mà không có feature tương ứng, các lệnh `multiclaw channel list`, `multiclaw channel doctor`, và `multiclaw channel start` sẽ ghi log rằng channel đó bị bỏ qua có chủ ý trong bản build này.
