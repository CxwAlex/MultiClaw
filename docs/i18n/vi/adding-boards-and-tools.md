# Thêm Board và Tool — Hướng dẫn phần cứng MultiClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào MultiClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.multiclaw/config.toml)
multiclaw peripheral add nucleo-f401re /dev/ttyACM0
multiclaw peripheral add arduino-uno /dev/cu.usbmodem12345
multiclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, MultiClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `multiclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `multiclaw peripheral list` | Liệt kê các board đã cấu hình |
| `multiclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `multiclaw peripheral flash` | Nạp firmware Arduino |
| `multiclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `multiclaw hardware discover` | Liệt kê thiết bị USB |
| `multiclaw hardware info` | Thông tin chip qua probe-rs |
