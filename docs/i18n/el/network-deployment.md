# Ανάπτυξη Δικτύου — MultiClaw σε Raspberry Pi και Τοπικό Δίκτυο

Αυτό το έγγραφο καλύπτει την ανάπτυξη του MultiClaw σε ένα Raspberry Pi ή σε άλλον κεντρικό υπολογιστή στο τοπικό σας δίκτυο, με κανάλια Telegram και προαιρετικά κανάλια webhook.
...
|------|----------------------|----------|
| **Telegram polling** | Όχι | Το MultiClaw αντλεί δεδομένα από το API του Telegram. Λειτουργεί από παντού. |
| **Matrix sync (συμπ. E2EE)** | Όχι | Το MultiClaw συγχρονίζεται μέσω του API του Matrix. Δεν απαιτείται εισερχόμενο webhook. |
...
**Σημείωση:** Τα Telegram, Discord, Slack και Nostr χρησιμοποιούν **εξερχόμενες συνδέσεις** — το MultiClaw συνδέεται σε εξωτερικούς διακομιστές. Δεν απαιτείται προώθηση θυρών (port forwarding) ή δημόσια IP.

---

## 2. MultiClaw σε Raspberry Pi
...
Επεξεργαστείτε το αρχείο `~/.multiclaw/config.toml`:
...
- Το κανάλι Telegram λειτουργεί: το MultiClaw αντλεί δεδομένα από το API του Telegram (εξερχόμενη σύνδεση).
...
2. Ξεκινήστε μια σήραγγα (tunnel):
   ...
   ```
   Ή χρησιμοποιήστε την εντολή `multiclaw tunnel`.

3. Το MultiClaw θα απορρίψει το `0.0.0.0` εκτός εάν η επιλογή `allow_public_bind = true` ή μια σήραγγα είναι ενεργή.
...
- Το MultiClaw καλεί το `https://api.telegram.org/bot{token}/getUpdates`.
...
Εκτελέστε το `multiclaw daemon` — το κανάλι Telegram ξεκινά αυτόματα.

Για την έγκριση ενός λογαριασμού Telegram κατά την εκτέλεση:

```bash
multiclaw channel bind-telegram <ΤΑΥΤΟΤΗΤΑ>
```
...
- Διατηρήστε μόνο μία ενεργή εκτέλεση για το ίδιο token (συνιστάται: η υπηρεσία `multiclaw daemon`).
...
- [ ] Εκτέλεση `multiclaw daemon --host 127.0.0.1 --port 42617`.
...
Το MultiClaw υποστηρίζει το OpenRC για το Alpine Linux και άλλες διανομές που χρησιμοποιούν το σύστημα αρχικοποίησης OpenRC. Οι υπηρεσίες OpenRC εκτελούνται **σε όλο το σύστημα** και απαιτούν δικαιώματα root/sudo.

### 7.1 Προαπαιτούμενα

- Alpine Linux (ή άλλη διανομή βασισμένη στο OpenRC).
- Πρόσβαση Root ή sudo.
- Ένας αποκλειστικός χρήστης συστήματος `multiclaw` (δημιουργείται κατά την εγκατάσταση).
...
Αυτό δημιουργεί:
- Σενάριο αρχικοποίησης (Init script): `/etc/init.d/multiclaw`
- Κατάλογο ρυθμίσεων: `/etc/multiclaw/`
- Κατάλογο καταγραφών (Logs): `/var/log/multiclaw/`

### 7.3 Ρύθμιση

Συνήθως δεν απαιτείται χειροκίνητη αντιγραφή των ρυθμίσεων. Η εντολή `sudo multiclaw service install` προετοιμάζει αυτόματα το `/etc/multiclaw`, μεταφέρει την υπάρχουσα κατάσταση από τις ρυθμίσεις του χρήστη σας και ορίζει τις σωστές άδειες για τον χρήστη της υπηρεσίας `multiclaw`.
...
| Εντολή | Περιγραφή |
|---------|-------------|
| `sudo rc-service multiclaw start` | Έναρξη του δαίμονα |
| `sudo rc-service multiclaw stop` | Διακοπή του δαίμονα |
| `sudo rc-service multiclaw status` | Έλεγχος κατάστασης υπηρεσίας |
| `sudo rc-service multiclaw restart` | Επανεκκίνηση του δαίμονα |
...
| Καταγραφή | Διαδρομή |
|-----|------|
| Πρόσβαση/stdout | `/var/log/multiclaw/access.log` |
| Σφάλματα/stderr | `/var/log/multiclaw/error.log` |
