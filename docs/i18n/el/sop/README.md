...
- Το CLI `multiclaw sop` διαχειρίζεται αυτή τη στιγμή μόνο ορισμούς: `list`, `validate`, `show`.
...
2. Δημιουργήστε έναν κατάλογο SOP, για παράδειγμα:
   ...
   ```text
   ~/.multiclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.multiclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Επαλήθευση και επιθεώρηση ορισμών:
   ...
   ```bash
   multiclaw sop list
   multiclaw sop validate
   multiclaw sop show deploy-prod
   ```
