# Προσθήκη Πλακετών και Εργαλείων — Οδηγός Υλικού MultiClaw

Αυτός ο οδηγός εξηγεί πώς να προσθέσετε νέες πλακέτες υλικού και προσαρμοσμένα εργαλεία στο MultiClaw.

## Γρήγορη Εκκίνηση: Προσθήκη Πλακέτας μέσω CLI

```bash
# Προσθήκη πλακέτας (ενημερώνει το ~/.multiclaw/config.toml)

multiclaw peripheral add nucleo-f401re /dev/ttyACM0
multiclaw peripheral add arduino-uno /dev/cu.usbmodem12345
multiclaw peripheral add rpi-gpio native   # για Raspberry Pi GPIO (Linux)
...
Με τη δυνατότητα rag-pdf, το MultiClaw μπορεί να ευρετηριάσει αρχεία PDF:
...
2. Προσθήκη στις ρυθμίσεις — multiclaw peripheral add my-board /dev/ttyUSB0
...
| Εντολή | Περιγραφή |
|---------|-------------|
| multiclaw peripheral list | Λίστα ρυθμισμένων πλακετών |
| multiclaw peripheral add <board> <path> | Προσθήκη πλακέτας (εγγραφή στο config) |
| multiclaw peripheral flash | Μεταφόρτωση (flash) υλικολογισμικού Arduino |
| multiclaw peripheral flash-nucleo | Μεταφόρτωση (flash) υλικολογισμικού Nucleo |
| multiclaw hardware discover | Λίστα συσκευών USB |
| multiclaw hardware info | Πληροφορίες chip μέσω probe-rs |
