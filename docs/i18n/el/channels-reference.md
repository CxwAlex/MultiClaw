...
Αυτός ο οδηγός περιγράφει τη διαδικασία διαμόρφωσης των καναλιών επικοινωνίας (Telegram, Discord, κ.λπ.) στο MultiClaw.
...
Οι ρυθμίσεις αποθηκεύονται στο αρχείο `~/.multiclaw/config.toml`, στην ενότητα `[channels_config]`.
...
Σε πλατφόρμες όπως το Telegram και το Discord, μπορείτε να ελέγχετε το MultiClaw με εντολές:
...
Το MultiClaw υποστηρίζει την ανάγνωση εικόνων μέσω της σύνταξης:
`[IMAGE:<source>]`
...
5. **Επανεκκίνηση**: Κάθε αλλαγή στο `config.toml` απαιτεί επανεκκίνηση του `multiclaw daemon`.

### Διαγνωστικά Μηνύματα (Logs)

Εκτελέστε το MultiClaw με αυξημένο επίπεδο καταγραφής:
```bash
