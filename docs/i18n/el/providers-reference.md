# Αναφορά Παρόχων MultiClaw (Providers Reference)
...
Το runtime του MultiClaw αναζητά διαπιστευτήρια με την εξής σειρά προτεραιότητας:
...
3. **Γενικές μεταβλητές**: Εφεδρικές μεταβλητές όπως οι `MULTICLAW_API_KEY` ή `API_KEY`.
...
- Τα μοντέλα συλλογιστικής (thinking models) υποστηρίζονται εγγενώς· το MultiClaw φιλτράρει αυτόματα τα εσωτερικά metadata της συλλογιστικής.
...
Κλήση μέσω CLI: `multiclaw agent --model hint:fast --message "..."`.
