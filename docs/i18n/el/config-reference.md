# Οδηγός Ρυθμίσεων MultiClaw (config.toml)
...
Το MultiClaw ψάχνει για τις ρυθμίσεις με την εξής σειρά:
1. Στη διαδρομή που ορίζει η μεταβλητή `MULTICLAW_WORKSPACE`.
2. Στο αρχείο `~/.multiclaw/config.toml` (αυτή είναι η συνηθισμένη θέση).
...
- Αν αλλάξετε το αρχείο `config.toml`, πρέπει να κάνετε επανεκκίνηση το MultiClaw για να δει τις αλλαγές.
- Χρησιμοποιήστε την εντολή `multiclaw doctor` για να βεβαιωθείτε ότι οι ρυθμίσεις σας είναι σωστές.
