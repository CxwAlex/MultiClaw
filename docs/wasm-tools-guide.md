...
(skills) in MultiClaw. WASM tools let you extend the agent with custom capabilities
written in any language that compiles to WebAssembly — without modifying MultiClaw's
core source code.
...
7. [How MultiClaw Loads and Uses the Tool](#7-how-multiclaw-loads-and-uses-the-tool)
...
└───────────────────────┬─────────────────────────────────────┘
                        │  WASI stdio protocol
┌───────────────────────▼─────────────────────────────────────┐
│  MultiClaw WASM engine (wasmtime + WASI)                     │
│                                                             │
...
| Requirement | Purpose |
|---|---|
| MultiClaw built with `--features wasm-tools` | Enables the WASM runtime |
| `wasmtime` CLI | Local testing (`multiclaw skill test`) |
...
```bash
multiclaw skill new <name> --template <typescript|rust|go|python>
```

Example:

```bash
multiclaw skill new weather_lookup --template rust
```
...
**Input** (written to the tool's stdin by MultiClaw):
...
`parameters`. MultiClaw passes the LLM-provided argument object verbatim.

**Output** (read from the tool's stdout by MultiClaw):
...
MultiClaw the tool's name, description, and the JSON Schema for its parameters.
...
| `homepage` | no | Optional URL shown in `multiclaw skill list` |
...
componentize-py -d wit/ -w multiclaw-skill componentize app -o tool.wasm
```
...
| Python | `componentize-py -d wit/ -w multiclaw-skill componentize app -o tool.wasm` | `tool.wasm` |
...
Before installing, test the tool directly without starting the full MultiClaw agent:

```bash
multiclaw skill test . --args '{"city":"Hanoi","units":"metric"}'
```

You can also test an installed skill by name:

```bash
multiclaw skill test weather_lookup --args '{"city":"Tokyo"}'
```

Or test a specific tool inside a multi-tool skill:

```bash
multiclaw skill test . --tool my_tool_name --args '{"city":"Paris"}'
```
...
```bash
multiclaw skill install ./weather_lookup
```

This copies your skill directory into `<workspace>/skills/weather_lookup/`.
MultiClaw will auto-discover it on next startup.

### 6.2 Install from a git repository

```bash
multiclaw skill install https://github.com/yourname/weather_lookup.git
```

MultiClaw clones the repository into the skills directory and scans for WASM tools.

### 6.3 Install from ZeroMarket registry

```bash
# Format: namespace/package-name
multiclaw skill install acme/weather-lookup

# With a specific version
multiclaw skill install acme/weather-lookup@0.2.1
```

MultiClaw fetches the package index from the configured registry URL, then downloads
`tool.wasm` and `manifest.json` for each tool in the package.

**Verify the install:**

```bash
multiclaw skill list
```

---

## 7. How MultiClaw Loads and Uses the Tool

### 7.1 Startup discovery

Every time the MultiClaw agent starts, it scans the `skills/` directory and loads
...
Agent: [internally, LLM selects tool "weather_lookup" with args {"city":"Hanoi"}]
       ...
       MultiClaw calls weather_lookup WASM tool:
         stdin  → {"city":"Hanoi"}
...
If a tool fails (non-zero exit, invalid JSON, timeout, fuel exhaustion), MultiClaw
...
**Installed layout** (created by `multiclaw skill install`):
...
Add this section to your `multiclaw.toml` to tune WASM tool behavior:
...
# Registry URL used by `multiclaw skill install namespace/package`
...
**Tool not appearing in `multiclaw skill list`**
