...
- **Quota Tracking**: `multiclaw providers-quota --provider qwen-code` shows static quota info (`?/1000` - unknown remaining, 1000/day total)
...
Since MultiClaw cannot auto-detect this, you can override it in `config.toml`:
...
Environment override: `MULTICLAW_MODEL_SUPPORT_VISION=true`
...
- When set, overrides `MULTICLAW_CODEX_REASONING_EFFORT`.
