...
- Environment override: `MULTICLAW_MODEL_SUPPORT_VISION` or `MODEL_SUPPORT_VISION` (values: `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off`).
...
- For OpenAI-compatible Responses fallback transport:
  - `MULTICLAW_RESPONSES_WEBSOCKET=1` forces websocket-first mode (`wss://.../responses`) for compatible providers.
  - `MULTICLAW_RESPONSES_WEBSOCKET=0` forces HTTP-only mode.
...
- When set, overrides `MULTICLAW_CODEX_REASONING_EFFORT` for OpenAI Codex requests.
- Unset falls back to `MULTICLAW_CODEX_REASONING_EFFORT` if present, otherwise defaults to `xhigh`.
...
| `user_agent` | `MultiClaw/1.0` | User-Agent header for outbound HTTP requests |
...
| `timeout_secs` | `30` | Request timeout in seconds |
| `user_agent` | `MultiClaw/1.0` | User-Agent header for fetch requests |
...
| `user_agent` | `MultiClaw/1.0` | User-Agent header for search requests |
...
Inter-process communication for independent MultiClaw agents on the same host.

| Key | Default | Purpose |
|---|---|---|
| `enabled` | `false` | Enable IPC tools (`agents_list`, `agents_send`, `agents_inbox`, `state_get`, `state_set`) |
| `db_path` | `~/.multiclaw/agents.db` | Shared SQLite database path (all agents on this host share one file) |
