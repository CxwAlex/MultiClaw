# Network Deployment — MultiClaw on Raspberry Pi and Local Network

This document covers deploying MultiClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | MultiClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | MultiClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — MultiClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. MultiClaw on Raspberry Pi
...
- Telegram channel works: MultiClaw polls Telegram API (outbound)
...
```bash
multiclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `multiclaw tunnel` (see tunnel docs).

3. MultiClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- MultiClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `multiclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `multiclaw daemon` service).
...
- [ ] Run `multiclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
MultiClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `multiclaw` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/multiclaw`
- Config directory: `/etc/multiclaw/`
- Log directory: `/var/log/multiclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo multiclaw service install` automatically prepares `/etc/multiclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `multiclaw` service user.

If no prior runtime state is available to migrate, create `/etc/multiclaw/config.toml` before starting the service.
...
# Check status
sudo rc-service multiclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service multiclaw start` | Start the daemon |
| `sudo rc-service multiclaw stop` | Stop the daemon |
| `sudo rc-service multiclaw status` | Check service status |
| `sudo rc-service multiclaw restart` | Restart the daemon |
| `sudo multiclaw service status` | MultiClaw status wrapper (uses `/etc/multiclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/multiclaw/access.log` |
| Errors/stderr | `/var/log/multiclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/multiclaw/error.log
```
...
sudo rc-service multiclaw stop
sudo rc-update del multiclaw default

# Remove init script
sudo multiclaw service uninstall
```
...
- The service runs as the `multiclaw:multiclaw` user (least privilege)
- Config must be at `/etc/multiclaw/config.toml` (explicit path in init script)
- If the `multiclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo multiclaw service install`
- [ ] Enable: `sudo rc-update add multiclaw default`
- [ ] Start: `sudo rc-service multiclaw start`
- [ ] Verify: `sudo rc-service multiclaw status`
- [ ] Check logs: `/var/log/multiclaw/error.log`
