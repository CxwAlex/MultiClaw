# OpenClaw → MultiClaw Migration Guide

This guide walks you through migrating an OpenClaw deployment to MultiClaw. It covers configuration conversion, endpoint changes, and the architectural differences you need to know.
...
# 2. The compatibility layer is built into MultiClaw — no files to copy.
...
MultiClaw is a **standalone messaging gateway**. It owns the full agent loop internally. Channels (WhatsApp, Linq, Nextcloud Talk) send a single message string, and MultiClaw handles everything: system prompt construction, tool invocation, memory recall, context enrichment, and response generation.
...
|----------|--------|------------|----------|
| `POST /api/chat` | MultiClaw-native JSON | Full (with tools + memory) | **Recommended** for new integrations |
...
The clean, MultiClaw-native endpoint.
...
OpenClaw stores state in `~/.openclaw/`. MultiClaw uses configurable backends:
...
| OpenClaw Channel | MultiClaw Status |
|------------------|-----------------|
...
For unsupported channels, point your existing integration at MultiClaw's `/api/chat` endpoint instead of OpenClaw's `/v1/chat/completions`.
...
```typescript
// Same code as before — just point to MultiClaw host with gateway token.
...
MultiClaw's `process_message` starts fresh on each call. It uses **semantic memory recall** (SQLite hybrid search with embeddings + BM25) to surface relevant past context, not ordered conversation history.
...
For full conversation history support, a follow-up change to MultiClaw's agent loop would be needed to accept a `messages[]` parameter directly.
...
# Specify output path
python scripts/convert-openclaw-config.py ~/.openclaw/openclaw.json -o ~/.multiclaw/config.toml
...
- [ ] Set API key: `export MULTICLAW_API_KEY='...'`
...
**405 on /v1/chat/completions:** The endpoint isn't registered. Make sure you're running a MultiClaw build that includes the `openclaw_compat` module (check `src/gateway/mod.rs` for the route registration).
