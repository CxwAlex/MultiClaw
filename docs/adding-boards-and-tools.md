# Adding Boards and Tools — MultiClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to MultiClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.multiclaw/config.toml)
multiclaw peripheral add nucleo-f401re /dev/ttyACM0
multiclaw peripheral add arduino-uno /dev/cu.usbmodem12345
multiclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, MultiClaw can index PDF files:
...
2. **Add to config** — `multiclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `multiclaw peripheral list` | List configured boards |
| `multiclaw peripheral add <board> <path>` | Add board (writes config) |
| `multiclaw peripheral flash` | Flash Arduino firmware |
| `multiclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `multiclaw hardware discover` | List USB devices |
| `multiclaw hardware info` | Chip info via probe-rs |
