...
```bash
curl -fsSL https://raw.githubusercontent.com/multiclaw-labs/multiclaw/main/scripts/install.sh | bash
```
...
it pulls `ghcr.io/multiclaw-labs/multiclaw:latest` and tags it locally before running.
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/multiclaw-labs/multiclaw:latest`)
- `--skip-install`
- `--provider <id>`
