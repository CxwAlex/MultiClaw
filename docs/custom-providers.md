...
- Auto mode: when your `custom:` endpoint resolves to `api.openai.com`, MultiClaw will try WebSocket mode first (`wss://.../responses`) and automatically fall back to HTTP if the websocket handshake or stream fails.
- Manual override:
  - `MULTICLAW_RESPONSES_WEBSOCKET=1` forces websocket-first mode for any `custom:` endpoint.
  - `MULTICLAW_RESPONSES_WEBSOCKET=0` disables websocket mode and uses HTTP only.
...
MultiClaw includes a first-class provider for [Tencent Hunyuan](https://hunyuan.tencent.com/):
