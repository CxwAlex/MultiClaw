...
1. Resolve latest workflow run IDs:
   - `gh run list --repo CxwAlex/multiclaw --workflow feature-matrix.yml --limit 1`
   - `gh run list --repo CxwAlex/multiclaw --workflow ci-run.yml --limit 1`
2. Enumerate check/job names and compare to this mapping:
   - `gh run view <run_id> --repo CxwAlex/multiclaw --json jobs --jq '.jobs[].name'`
