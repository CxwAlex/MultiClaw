...
This guide explains how to run MultiClaw reliably in Matrix rooms, including end-to-end encrypted (E2EE) rooms.
...
3. `room_id` is correct:
   ...
   - supported: room alias (`#alias:server`) and MultiClaw will resolve it
...
- MultiClaw attempts to read identity from Matrix `/_matrix/client/v3/account/whoami`.
...
3. Confirm MultiClaw logs contain Matrix listener startup and no repeated sync/auth errors.
...
- MultiClaw sends Matrix text replies as markdown-capable `m.room.message` text content.
- Matrix clients that support `formatted_body` should render emphasis, lists, and code blocks.
- If formatting appears as plain text, check client capability first, then confirm MultiClaw is running a build that includes markdown-enabled Matrix output.
