# MultiClaw Plugin System

A plugin architecture for MultiClaw modeled after [OpenClaw's plugin system](https://github.com/openclaw/openclaw), adapted for Rust.

## Overview

The plugin system allows extending MultiClaw with custom tools, hooks, channels, and providers without modifying the core codebase. Plugins are discovered from standard directories, loaded at startup, and registered with the host through a clean API.

## Architecture

### Key Components

1. **Manifest** (`multiclaw.plugin.toml`): Declares plugin metadata (id, name, version, description)
...
| OpenClaw (TypeScript)              | MultiClaw (Rust)                    |
|------------------------------------|------------------------------------|
| `openclaw.plugin.json`             | `multiclaw.plugin.toml`             |
...
`extensions/hello-world/multiclaw.plugin.toml`:

```toml
id = "hello-world"
name = "Hello World"
description = "Example plugin demonstrating the MultiClaw plugin API."
version = "0.1.0"
```
...
```rust
use multiclaw::plugins::{Plugin, PluginApi, PluginManifest};
use multiclaw::tools::traits::{Tool, ToolResult};
...
#[async_trait]
impl multiclaw::hooks::HookHandler for HelloHook {
...
```rust
use multiclaw::plugins::{load_plugins, Plugin};
...
2. **Global**: `~/.multiclaw/extensions/`
3. **Workspace**: `<workspace>/.multiclaw/extensions/`
4. **Custom**: Paths in `plugins.load_paths`

Each directory is scanned for subdirectories containing `multiclaw.plugin.toml`.
...
- A bad plugin won't crash MultiClaw
...
Implement `multiclaw::hooks::HookHandler`:
...
- **Hot reload**: Reload plugins without restarting MultiClaw
...
- [Issue #1414](https://github.com/multiclaw-labs/multiclaw/issues/1414)
