# MultiClaw on Nucleo-F401RE — Step-by-Step Guide

Run MultiClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

MultiClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
MultiClaw includes everything for Nucleo-F401RE:
...
| Flash command | `multiclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via MultiClaw

From the multiclaw repo root:

```bash
multiclaw peripheral flash-nucleo
```

This builds `firmware/multiclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `multiclaw daemon` or `multiclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `multiclaw peripheral list` to verify.
