...
One MultiClaw runtime can serve multiple channels at once: if you configure several
channel sub-tables, `multiclaw channel start` launches all of them in the same process.
...
When running `multiclaw channel start` (or daemon mode), runtime commands include:
...
- If image download fails, MultiClaw forwards fallback text instead of silently dropping the message.
