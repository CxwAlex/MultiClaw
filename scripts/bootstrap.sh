...
Options:
  ...
  --docker-reset             Reset existing MultiClaw Docker containers/networks/volumes and data dir before --docker bootstrap
...
Examples:
  ...
  ./multiclaw_install.sh --docker --docker-config ./config.toml --docker-daemon

  # Compatibility entrypoint:
  ./bootstrap.sh --docker

  # Remote one-liner
  curl -fsSL https://raw.githubusercontent.com/multiclaw-labs/multiclaw/main/scripts/bootstrap.sh | bash
...
Environment:
  ...
  MULTICLAW_DOCKER_BROWSER_RUNTIME
                            Browser runtime provisioning mode for --docker: "auto" (prompt), "on", or "off"
  MULTICLAW_DOCKER_BROWSER_SIDECAR_IMAGE
                            Browser WebDriver sidecar image (default: selenium/standalone-chromium:latest)
  MULTICLAW_DOCKER_BROWSER_SIDECAR_NAME
                            Browser WebDriver sidecar container name (default: multiclaw-browser-webdriver)
  MULTICLAW_DOCKER_NETWORK    Docker network for MultiClaw + sidecars (default: multiclaw-bootstrap-net)
  MULTICLAW_DOCKER_CARGO_FEATURES
                            Extra Cargo features for Docker builds (comma-separated)
  MULTICLAW_CARGO_FEATURES    Extra Cargo features for local source builds (comma-separated)
  MULTICLAW_CONFIG_PATH       Config path used for channel feature auto-detection (default: ~/.multiclaw/config.toml)
  MULTICLAW_DOCKER_DAEMON_NAME
                            Daemon container name for --docker-daemon (default: multiclaw-daemon)
  MULTICLAW_DOCKER_DAEMON_BIND_HOST
                            Host bind address for daemon port publish (default: 127.0.0.1)
  MULTICLAW_DOCKER_DAEMON_HOST_PORT
                            Host port to publish daemon gateway (default: same as gateway.port)
  MULTICLAW_DOCKER_SECRET_KEY_FILE
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/multiclaw-labs/multiclaw/releases/latest/download/multiclaw-${target}.tar.gz"
...
is_multiclaw_container() {
  ...
  [[ "$name_lc" == *"multiclaw"* || "$image_lc" == *"multiclaw"* || "$command_lc" == *"multiclaw"* ]]
}

is_multiclaw_resource_name() {
  local name="$1"
  local name_lc
  name_lc="$(printf '%s' "$name" | tr '[:upper:]' '[:lower:]')"
  [[ "$name_lc" == *"multiclaw"* ]]
...
maybe_stop_running_multiclaw_containers() {
  ...
  while IFS=$'\t' read -r id name image command; do
    ...
    if is_multiclaw_container "$name" "$image" "$command"; then
      running_ids+=("$id")
  ...
  warn "Detected running MultiClaw container(s):"
  ...
  if ! guided_input_stream >/dev/null 2>&1; then
    warn "Non-interactive mode detected; leaving existing MultiClaw containers running."
    return 0
  fi

  if prompt_yes_no "Stop these running MultiClaw containers before continuing?" "yes"; then
    info "Stopping ${#running_ids[@]} MultiClaw container(s)"
    "$CONTAINER_CLI" stop "${running_ids[@]}" >/dev/null
  else
    warn "Continuing with existing MultiClaw containers still running."
  fi
}

reset_docker_multiclaw_resources() {
  ...
  info "Resetting MultiClaw Docker resources"
  ...
  if [[ ${#container_ids[@]} -gt 0 ]]; then
    info "Removing ${#container_ids[@]} MultiClaw container(s)"
    ...
    info "No existing MultiClaw containers found"
  fi
  ...
  while IFS= read -r resource_name; do
    ...
    if is_multiclaw_resource_name "$resource_name"; then
      network_names+=("$resource_name")
    fi
  done < <("$CONTAINER_CLI" network ls --format '{{.Name}}')

  if [[ ${#network_names[@]} -gt 0 ]]; then
    info "Removing ${#network_names[@]} MultiClaw network(s)"
    ...
    info "No existing MultiClaw networks found"
  fi
  ...
  if [[ ${#volume_names[@]} -gt 0 ]]; then
    info "Removing ${#volume_names[@]} MultiClaw volume(s)"
    ...
    info "No existing MultiClaw volumes found"
  fi
...
run_docker_bootstrap() {
  ...
  fallback_image="ghcr.io/multiclaw-labs/multiclaw:latest"
  docker_build_features="${MULTICLAW_DOCKER_CARGO_FEATURES:-}"
  docker_browser_runtime_mode="${MULTICLAW_DOCKER_BROWSER_RUNTIME:-auto}"
  docker_browser_sidecar_name="${MULTICLAW_DOCKER_BROWSER_SIDECAR_NAME:-multiclaw-browser-webdriver}"
  docker_browser_sidecar_image="${MULTICLAW_DOCKER_BROWSER_SIDECAR_IMAGE:-selenium/standalone-chromium:latest}"
  docker_network="${MULTICLAW_DOCKER_NETWORK:-multiclaw-bootstrap-net}"
  docker_daemon_name="${MULTICLAW_DOCKER_DAEMON_NAME:-multiclaw-daemon}"
  docker_daemon_bind_host="${MULTICLAW_DOCKER_DAEMON_BIND_HOST:-127.0.0.1}"
  docker_daemon_host_port="${MULTICLAW_DOCKER_DAEMON_HOST_PORT:-}"
  seed_config_path="${DOCKER_CONFIG_FILE:-}"
  seed_secret_key_path="${DOCKER_SECRET_KEY_FILE:-${MULTICLAW_DOCKER_SECRET_KEY_FILE:-}}"
  ...
  if [[ "$DOCKER_RESET" == true ]]; then
    reset_docker_multiclaw_resources "$docker_data_dir"
  fi
  ...
  if [[ -n "$seed_config_path" ]]; then
    ...
    install -m 600 "$seed_config_path" "$docker_data_dir/.multiclaw/config.toml"
    seed_docker_secret_key_for_config "$seed_config_path" "$docker_data_dir/.multiclaw" "$seed_secret_key_path"
  fi
  ...
  case "$(printf '%s' "$docker_browser_runtime_mode" | tr '[:upper:]' '[:lower:]')" in
    ...
    *)
      ...
      if [[ "$docker_browser_runtime_bool" == "invalid" ]]; then
        warn "Invalid MULTICLAW_DOCKER_BROWSER_RUNTIME='$docker_browser_runtime_mode' (expected auto/on/off). Defaulting to off."
        docker_browser_runtime_bool="false"
      fi
      ;;
  esac
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if [[ -n "$docker_build_features" ]]; then
      ...
      docker_build_args+=(--build-arg "MULTICLAW_CARGO_FEATURES=$docker_build_features")
    fi
  ...
  if [[ -n "$docker_browser_webdriver_url" ]]; then
    container_extra_env_args+=(-e "MULTICLAW_DOCKER_WEBDRIVER_URL=$docker_browser_webdriver_url")
  fi
  ...
  if [[ "$DOCKER_DAEMON_MODE" == true ]]; then
    ...
    if [[ -f "$docker_data_dir/.multiclaw/config.toml" ]]; then
      config_gateway_port_value="$(config_gateway_port "$docker_data_dir/.multiclaw/config.toml" || true)"
    fi
    ...
    daemon_cmd+=(
      ...
      -e MULTICLAW_DOCKER_BOOTSTRAP=1
    )
...
REPO_URL="https://github.com/multiclaw-labs/multiclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
LOCAL_CARGO_FEATURES="${MULTICLAW_CARGO_FEATURES:-}"
LOCAL_CONFIG_PATH="${MULTICLAW_CONFIG_PATH:-$HOME/.multiclaw/config.toml}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  if [[ -n "$LOCAL_CARGO_FEATURES" ]]; then
    warn "--cargo-features / MULTICLAW_CARGO_FEATURES are ignored with --docker (use MULTICLAW_DOCKER_CARGO_FEATURES)."
  fi
  ensure_docker_ready
  ...
  echo
  echo "Your containerized MultiClaw data is persisted under:"
  echo "  $DOCKER_DATA_DIR"
  echo

  if [[ "$DOCKER_DAEMON_MODE" == true ]]; then
    daemon_name="${MULTICLAW_DOCKER_DAEMON_NAME:-multiclaw-daemon}"
    ...
    echo "  ./multiclaw_install.sh --docker --interactive-onboard"
  elif [[ "$RUN_ONBOARD" == false ]]; then
    echo "Onboarding was intentionally skipped (pre-seeded config mode)."
    echo
    echo "Next steps:"
    echo "  ./multiclaw_install.sh --docker --docker-config ./config.toml --docker-daemon"
    echo "  ./multiclaw_install.sh --docker --interactive-onboard"
  else
    cat <<'DONE'
Next steps:
