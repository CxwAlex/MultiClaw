#!/usr/bin/env python3
"""
OpenClaw → MultiClaw Configuration Converter

Reads an OpenClaw config (openclaw.json) and produces a MultiClaw config (config.toml).
...
# MultiClaw splits these into default_provider + default_model
PROVIDER_MAP = {
    "anthropic": "anthropic",
    "openai": "openai",
...
# OpenClaw model names → MultiClaw model names (when they differ)
MODEL_MAP = {
...
# Channel name mapping (OpenClaw → MultiClaw config section)
CHANNEL_MAP = {
    "whatsapp": "whatsapp",
    "telegram": "telegram",
    "discord": "discord",
    "slack": "slack",
    "signal": None,          # Not natively supported in MultiClaw
    "imessage": None,        # Not natively supported in MultiClaw
    "googlechat": None,      # Not natively supported in MultiClaw
    "msteams": None,         # Not natively supported in MultiClaw
    "matrix": "matrix",
    "webchat": None,         # Handled differently — use /api/chat
    "bluebubbles": None,     # Not natively supported in MultiClaw
    "zalo": None,            # Not natively supported in MultiClaw
    "lark": "lark",
    "feishu": "feishu",
...
def convert_gateway(oc: dict) -> dict:
    ...
    # Port mapping: OpenClaw default 18789, MultiClaw default 42617
    if "port" in gw:
        result["port"] = gw["port"]
    else:
        result["port"] = 42617  # MultiClaw default
...
def convert_memory(oc: dict) -> dict:
    ...
    result = {
        "backend": "sqlite",  # MultiClaw default, best match for OpenClaw behavior
        "auto_save": True,
...
def build_toml(oc: dict) -> str:
    """Build MultiClaw config.toml from parsed OpenClaw config."""
    lines = []
    lines.append("# MultiClaw configuration")
    ...
    lines.append("# API key — set via MULTICLAW_API_KEY env var or uncomment below:")
    ...
    if unsupported:
        lines.append("# WARNING: The following OpenClaw channels are not natively supported in MultiClaw:")
...
def generate_migration_notes(oc: dict) -> str:
    ...
    if unsupported:
        ...
        notes.append("   These channels don't have native MultiClaw integrations.")
        ...
        notes.append("     3. Check MultiClaw's community channels for third-party integrations")

    # Check for skills
    if oc.get("skills"):
        notes.append("")
        notes.append("⚠  SKILLS: OpenClaw skills (UV Python scripts) are not directly compatible.")
        notes.append("   MultiClaw skills use a different format. You'll need to port them.")
        notes.append("   See: docs/skills-migration.md (if available)")

    # Check for workflows
    if oc.get("workflows"):
        notes.append("")
        notes.append("⚠  WORKFLOWS: OpenClaw workflows need to be converted to MultiClaw's")
    ...
    notes.append("     export MULTICLAW_API_KEY='sk-...'")
    notes.append("   Or uncomment the api_key line in config.toml")

    # Pairing
    notes.append("")
    notes.append("🔐 PAIRING:")
    notes.append("   Run your MultiClaw instance and pair using:")
    ...
    notes.append("   OpenClaw endpoint         → MultiClaw equivalent")
    notes.append("   ─────────────────────────────────────────────────")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert OpenClaw config to MultiClaw format"
    )
    parser.add_argument(
        "input",
